...
class Warp(QThread):
    ...
    def __init__(self, image1Info, image2Info):
    ...
    def run(self):
        ...
        for i in range(self.frame_count):
            print("Start %d warp ~" % (i+1))
            self.runWarp(i)
            self.sinOut.emit('%d/10 done' % (i+1))
...
class present_Window(QWidget):
    ...
    def __init__(self, widget, image1Info, image2Info):
        ...
        self.dick = Warp(image1Info, image2Info)
        self.dick.start()

        self.dick.sinOut.connect(self.doneWarp)
        self.dick.finished.connect(self.thread_finished)
...
class Draw_Line_Window(QWidget):
    ...
    def nextStep(self):
        ...
        window3 = present_Window(self.widget, self.image1Info, self.image2Info)
